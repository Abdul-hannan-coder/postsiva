...
import useInstagramAuth from '@/lib/hooks/auth/platforms/instagram/useInstagramAuth';
...
export default function InstagramPage() {
  ...
  const { isLoading, error, initiateInstagramConnect } = useInstagramAuth();

  const handleConnect = async () => {
    if (!isAuthenticated) {
      window.location.href = '/auth/signin?next=/platforms/instagram';
      return;
    }
    try {
      ...
      {showConnectForm && (
          ...
          <div className="bg-gray-900 rounded-2xl p-8 max-w-md w-full border border-gray-800 relative">
            ...
            <div className="space-y-4">
              ...
              >
                {isLoading ? 'Opening Instagram...' : 'Authorize Instagram Access'}
              </button>
              <button
                onClick={() => setShowConnectForm(false)}
