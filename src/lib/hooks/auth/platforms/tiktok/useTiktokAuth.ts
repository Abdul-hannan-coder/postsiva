...
import type { TikTokCreateTokenResponse } from './types/tiktokTypes';
...
export default function useTiktokAuth() {
  ...
  const initiateTikTokConnect = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.post<TikTokCreateTokenResponse>(
        '/tiktok/create-token',
        {},
        {
          headers: {
      ...
      if (DEBUG_LOGS) console.log('🎵 TikTok create-token response:', res.data);
      ...
      if (res.data.success && authUrl) {
        ...
        if (!popup || popup.closed || typeof popup.closed === 'undefined') {
          if (DEBUG_LOGS) console.warn('Popup blocked; redirecting current tab to TikTok');
          window.location.href = authUrl;
        } else {
          try { popup.focus(); } catch {}
        }
      } else {
        throw new Error(res.data?.message || 'Failed to get TikTok auth URL');
      }
    } catch (err: any) {
      const message = err?.message || 'Failed to initiate TikTok auth';
      dispatch({ type: 'SET_ERROR', payload: message });
      if (DEBUG_LOGS) console.error('❌ TikTok connect error:', err);
      throw err;
    } finally {
